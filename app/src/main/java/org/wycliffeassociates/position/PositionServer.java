...
import org.wycliffeassociates.position.Network.*;
...
public class PositionServer implements ServerDiscoveryHandler {
	...
	public PositionServer (Context c, String serverName) throws IOException {
		context = c;
		server = new Server() {
		...
		server.addListener(new Listener() {
			...
			public void received (Connection c, Object object) {
				...
				if (object instanceof Register) {
					...
					character.color = Color.argb(255, rnd.nextInt(256), rnd.nextInt(256), rnd.nextInt(256));
	...
	boolean saveCharacter (Character character) {
		File file = new File(context.getFilesDir(), character.name.toLowerCase());
	...
	Character loadCharacter (String name) {
		File file = new File(context.getFilesDir(), name.toLowerCase());
